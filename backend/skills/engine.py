    ...
    async def _execute_inner(
        ...
        for path in sorted_paths:
            if path.mode == "react_freeform":
                llm_max_rounds = skill.llm_config.max_rounds if skill.llm_config.max_rounds > 0 else path.max_rounds
                logger.info(
                    f"[SkillEngine] 🤖 所有确定性路径已尝试，进入 LLM 自由推理兜底 "
                    f"(限定在 {skill.name} 范围内，最多 {llm_max_rounds} 轮)"
                )
                return await self._react_freeform(
                    skill, finding, ctx, llm_max_rounds
                )
